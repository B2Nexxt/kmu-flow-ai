-- Migration 3.1b (Teil 1): RPC create_anfrageeingang
--
-- Neue Objekte:
--   public.create_anfrageeingang(...)
--
-- Unverändert: Tabellenstrukturen, /admin-RPCs, andere operative Tabellen
-- Keine GRANTs an anon/authenticated — ausschließlich Service Role serverseitig.
--
-- Referenz: docs/fachkonzept/15-spezifikation-m31-anfrageeingang-serverlogik.md
--           ADR-0018, ADR-0019

-- ---------------------------------------------------------------------------
-- RPC: create_anfrageeingang
-- ---------------------------------------------------------------------------
...
comment on function public.create_anfrageeingang(
  ...
  'Idempotent bei gleicher mandant_id+kanal+kanal_externe_id. Rückgabe jsonb mit ok/code. '
  'Nur serverseitig über Service Role (ADR-0019).';
