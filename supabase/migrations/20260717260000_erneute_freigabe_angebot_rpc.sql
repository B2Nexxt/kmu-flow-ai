...
-- Erweiterung: Erneute Freigabe einer offenen Version ohne neue Angebotsnummer.
...
begin
  ...
  elsif v_status = 'freigegeben' then
    v_erstfreigabe := false;

    if v_angebotsnummer is null then
      raise exception 'Angebotsnummer fehlt für erneute Freigabe';
    end if;
  else
    raise exception 'Freigabe nicht möglich bei Status: %', v_status;
  end if;
